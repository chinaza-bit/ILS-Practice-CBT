(() => {
  ...
  let state = {questions:[], index:0, answers:{}, flags:new Set(), secondsLeft:0, totalSeconds:0, elapsedSeconds:0, timerId:null, submitted:false, student:'', perQuestion:10};
  const letters=['A','B','C','D'];
  const countSelect=$('#countSelect'), timeSelect=$('#timeSelect');
  ...
  function updatePreview(){const n=+countSelect.value,sec=+timeSelect.value,total=n*sec;$('#timePreview').textContent=`${Math.floor(total/60)}m ${total%60}s total`;}
  countSelect.addEventListener('change',updatePreview);timeSelect.addEventListener('change',updatePreview);updatePreview();

  function startTest(){
    const count=+countSelect.value, perQuestion=+timeSelect.value;
    state.questions=prepareQuestions(shuffle([...QUESTION_BANK]).slice(0,Math.min(count,QUESTION_BANK.length)));
    state.index=0;state.answers={};state.flags=new Set();state.student=$('#studentName').value.trim()||'Student';state.perQuestion=perQuestion;state.secondsLeft=perQuestion;state.totalSeconds=count*perQuestion;state.elapsedSeconds=0;state.submitted=false;
  ...
  function tick(){
    ...
    if(state.secondsLeft<=0){
      if(state.index<state.questions.length-1){state.index++;state.secondsLeft=state.perQuestion;$('#timer').style.color='';renderQuestion();}
      else{clearInterval(state.timerId);submitTest(true)}
    }
  }
  ...
  function buildNavigator(){const g=$('#questionGrid');g.innerHTML=state.questions.map((q,i)=>`<button class="q-dot ${i===state.index?'current':''} ${state.answers[q.id]?'answered':''} ${state.flags.has(q.id)?'flagged':''}" data-i="${i}">${i+1}</button>`).join('');g.querySelectorAll('.q-dot').forEach(b=>b.addEventListener('click',()=>{state.index=+b.dataset.i;state.secondsLeft=state.perQuestion;$('#timer').style.color='';renderQuestion()}))}
  ...
  function renderResults(correct,wrong,unanswered){const total=state.questions.length,pct=Math.round(correct/total*100),used=state.elapsedSeconds;$('#resultTitle').textContent=pct>=70?'Excellent work!':pct>=50?'Good effort!':'Keep practising!';$('#resultSubtitle').textContent=`${state.student}, you completed ${total} GST 212 practice questions with ${state.perQuestion} seconds per question.`;$('#scorePercent').textContent=`${pct}%`;$('#correctStat').textContent=correct;$('#wrongStat').textContent=wrong;$('#unansweredStat').textContent=unanswered;$('#timeUsedStat').textContent=`${Math.floor(used/60)}m ${used%60}s`;$('#scoreFraction').textContent=`${correct} / ${total}`;$('#scoreBar').style.width=`${pct}%`;$('#performanceMessage').textContent=pct>=70?'Strong performance. Review the missed questions to reinforce the details.':pct>=50?'You are on the right track. Revisit the questions marked wrong and try another batch.':'Use the review below as a study guide, then take the test again.';document.querySelector('.score-ring').style.background=`conic-gradient(var(--primary) ${pct}%, #dfeae7 ${pct}%)`;renderReview('all')}
  ...
  $('#startBtn').addEventListener('click',startTest);$('#prevBtn').addEventListener('click',()=>{if(state.index>0){state.index--;state.secondsLeft=state.perQuestion;$('#timer').style.color='';renderQuestion()}});$('#nextBtn').addEventListener('click',()=>{if(state.index<state.questions.length-1){state.index++;state.secondsLeft=state.perQuestion;$('#timer').style.color='';renderQuestion()}else{submitTest(false)}});$('#flagBtn').addEventListener('click',()=>{const id=state.questions[state.index].id;state.flags.has(id)?state.flags.delete(id):state.flags.add(id);renderQuestion()});$('#submitBtn').addEventListener('click',()=>submitTest(false));$('#reviewFilter').addEventListener('change',e=>renderReview(e.target.value));$('#newTestBtn').addEventListener('click',()=>{clearInterval(state.timerId);show('start')});$('#resetBtn').addEventListener('click',()=>{clearInterval(state.timerId);state={questions:[],index:0,answers:{},flags:new Set(),secondsLeft:0,totalSeconds:0,elapsedSeconds:0,timerId:null,submitted:false,student:'',perQuestion:10};show('start')});$('#themeBtn').addEventListener('click',()=>document.body.classList.toggle('dark'));
